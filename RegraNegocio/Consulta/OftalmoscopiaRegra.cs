...
{
    public class OftalmoscopiaRegra
    {
            ...
            string relacaoAVOd, string relacaoAVOe, string corOd, string corOe, string lenteOd, string lenteOe, string observacaoOftalmoscopia)
        {
            try
            {
                ...
                {
                   OftalmoscopiaAcesso CadastrarAcesso = new OftalmoscopiaAcesso();
            ...
            {
                MessageBox.Show("Ocorreu um erro ao cadastrar a oftalmoscopia(Classe OftalmoscopiaRegra, Método Cadastrar)", "Erro de Cadastro",MessageBoxButtons.OK,MessageBoxIcon.Error);
            }
            return false;
        }
           ...
           string relacaoAVOd, string relacaoAVOe, string corOd, string corOe, string lenteOd, string lenteOe, string observacaoOftalmoscopia)
        {
            ...
            {
                MessageBox.Show("Ocorreu um erro ao validar a oftalmoscopia(Classe OftalmoscopiaRegra, Método validar)", "Erro de validação", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return false;
        }
          ...
          string relacaoAVOd, string relacaoAVOe, string corOd, string corOe, string lenteOd, string lenteOe, string observacaoOftalmoscopia)
        {
            try
            {
                if (Validar(distanciaBruckner, papilaOd, papilaOe, escavacaoOd, escavacaoOe, maculaOd, maculaOe, fixacaoOd, ficacaoOe, relacaoAVOd, relacaoAVOe, corOd, corOe, lenteOd, lenteOe, observacaoOftalmoscopia) == true)
                {
                    OftalmoscopiaAcesso editarAcesso = new OftalmoscopiaAcesso();
            ...
            {
                MessageBox.Show("Ocorreu um erro ao atualizar a Oftalmoscopia(Classe OftalmoscopiaRegra, Método Editar)", "Erro de atualização/edição", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return false;
        }
        public DataTable LocalizarOftalmoscopia(string idOftalmoscopia)
        {
            try
            {
                ...
                OftalmoscopiaAcesso pesquisar = new OftalmoscopiaAcesso();
                if (Convert.ToInt32(idOftalmoscopia) > 0) dadosTabela = pesquisar.PesquisarOftalmoscopia(idOftalmoscopia);
                return dadosTabela;
            }
            catch (Exception)
            {
                MessageBox.Show("Ocorreu um erro ao pesquisar pela Oftalmoscopia(Classe OftalmoscopiaRegra, Método LocalizarOftalmoscopia)", "Erro de pesquisa",
