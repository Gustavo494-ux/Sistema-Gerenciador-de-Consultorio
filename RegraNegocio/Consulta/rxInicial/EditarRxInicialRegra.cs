...
namespace RegraNegocio
{
    ...
    public class EditarRxInicialRegra
    {
        public bool Editar(int idRxInicial, string vlOdEsfera, string vlOeEsfera, string vpOdEsfera, string vpOeEsfera, string vlOdCilindro, string vlOeCilindro, string vpOdCilindro, string vpOeCilindro, string vlOdEixo,
            ...
            string vlOdBase, string vlOeBase, string vpOdBase, string vpOeBase, string tipoLente, string material, string cor, string estado, string observacaoRxInicial)
        {
            try
            {
                if (Validar(idRxInicial, vlOdEsfera, vlOeEsfera, vpOdEsfera, vpOeEsfera, vlOdCilindro, vlOeCilindro, vpOdCilindro, vpOeCilindro, vlOdEixo, vlOeEixo, vpOdEixo, vpOeEixo, vlOdAdicao, vlOeAdicao, vpOdAdicao, vpOeAdicao, vlOdPrisma,
                vlOePrisma, vpOdPrisma, vpOePrisma, vlOdBase, vlOeBase, vpOdBase, vpOeBase, tipoLente, material, cor, estado, observacaoRxInicial) == true)
                {
                    EditarRxInicialAcesso EditarAcesso = new EditarRxInicialAcesso();
                    if (EditarAcesso.EditarRxInicial(Convert.ToString(idRxInicial), vlOdEsfera, vlOeEsfera, vpOdEsfera, vpOeEsfera, vlOdCilindro, vlOeCilindro, vpOdCilindro, vpOeCilindro, vlOdEixo, vlOeEixo, vpOdEixo, vpOeEixo, vlOdAdicao, vlOeAdicao, vpOdAdicao, vpOeAdicao, vlOdPrisma,
                    vlOePrisma, vpOdPrisma, vpOePrisma, vlOdBase, vlOeBase, vpOdBase, vpOeBase, tipoLente, material, cor, estado, observacaoRxInicial) == true) return true; return false;
                }
                else return false;
            }
            catch (Exception)
            {
                MessageBox.Show("Ocorreu um erro ao atualizar o Rx Inicial(Classe EditarRxInicialRegra, Método Editar)", "Erro de atualização/edição", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            return false;
        }
        public bool Validar(int idRxInicial, string vlOdEsfera, string vlOeEsfera, string vpOdEsfera, string vpOeEsfera, string vlOdCilindro, string vlOeCilindro, string vpOdCilindro, string vpOeCilindro, string vlOdEixo,
           string vlOeEixo, string vpOdEixo, string vpOeEixo, string vlOdAdicao, string vlOeAdicao, string vpOdAdicao, string vpOeAdicao, string vlOdPrisma, string vlOePrisma, string vpOdPrisma, string vpOePrisma,
           string vlOdBase, string vlOeBase, string vpOdBase, string vpOeBase, string tipoLente, string material, string cor, string estado, string observacaoRxInicial)
        {
            try
            {
                if (idRxInicial.GetType() != typeof(int) || idRxInicial <= 0) return false;
                ...
                if (observacaoRxInicial.GetType() != typeof(string) || observacaoRxInicial.Length > 50) return false;

                return true;
            }
            catch (Exception)
            {
                MessageBox.Show("Ocorreu um erro ao validar o Rx Inicial(Classe EditarRxInicialRegra, Método Validar)", "Erro de validação", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            return false;
        }
